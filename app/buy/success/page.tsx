...
export default async function SuccessPage({ searchParams }: Props) {
  ...
  let tx: Awaited<ReturnType<typeof verifyTransaction>> | null = null;
  ...
  if (txError || !tx) return <FailPage message={txError ?? "Payment verification failed."} />;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="bg-white rounded-2xl border border-red-200 p-8 max-w-sm w-full text-center">
        ...
        <Link href="/buy" className="inline-block bg-blue-600 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-blue-700">
          Try again
        </Link>
      </div>
    </div>
